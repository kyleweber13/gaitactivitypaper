...
def plot_section(data, start=None, stop=None, wrist_epochlen=15, incl_wrist_scatter=False, abs_time=False,
                 ...
                 use_epoch_hr=False, use_b2b_hr=True, figsize=(13, 6)):

    fig, ax = plt.subplots(3, sharex='col', figsize=figsize)
...
fig = plot_section(data=data, start='2020-03-03 15:28:45', stop='2020-03-03 15:35:46', abs_time=True,
                   ...
                   shade_long_walks=False, use_hrmax=True, use_hrr=False,
                   use_epoch_hr=False, use_b2b_hr=True, figsize=(18.27, 10.33))

label_fontsize, legend_fontsize, tick_fontsize = 16, 15, 15
...
fig.axes[0].set_ylabel("Wrist AVM (15-sec)", fontsize=label_fontsize)
fig.axes[1].set_ylabel("Cadence (steps/min)", fontsize=label_fontsize)
fig.axes[2].set_ylabel("Heart Rate (bpm)", fontsize=label_fontsize)
fig.axes[2].set_xlabel("Minutes", fontsize=label_fontsize)

for i in range(3):
    fig.axes[i].set_xticklabels([int(j) for j in fig.axes[i].get_xticks()], fontsize=tick_fontsize)
    fig.axes[i].legend(fontsize=legend_fontsize)

plt.tight_layout()
# plt.savefig("C:/Users/ksweber/Desktop/CSEP_plots/Raw/sample_window_large.tiff", dpi=200)
