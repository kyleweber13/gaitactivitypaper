...
import pyedflib
import nwgait
from DataImport import import_edf


def remove_toosoon_steps(df_steps, min_step_time=60/200):
    last_step = df_steps['step_time'].iloc[0]
    idx = []
    for i, t in enumerate(list(df_steps['step_time'])):
        dt = (t - last_step).total_seconds()

        if dt >= min_step_time:
            last_step = t
            idx.append(i)

    print(f"     -Removed {df_steps.shape[0] - len(idx)} steps")

    df_steps = df_steps.iloc[idx, :]
...
def create_ankle_edf_filenames(df):
    ...
    for row in df.itertuples():
        ...
        file_found = False
        if os.path.exists(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], row.UseSide)):
            ankle_fnames.append(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], row.UseSide))
            file_found = True

        else:
            if os.path.exists(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "L" if row.UseSide == "R" else "R")):
                if not file_found:
                    ankle_fnames.append(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "L" if row.UseSide == "R" else "R"))
                    file_found = True

            if os.path.exists(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "R" if row.UseSide == "R" else "L")):
                if not file_found:
                    ankle_fnames.append(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "R" if row.UseSide == "R" else "L"))
                    file_found = True

            if not os.path.exists(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "R")) and\
                    not os.path.exists(filepath + ankle_fname.format(row.full_id, fname.split("_")[3], "L")):
                if not file_found:
